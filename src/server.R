...
##Console##
{
    ...
    observe({
        console$.js <- input$JS
        .Q$toConsole(input$JS,'js-out','javascript')   
    })
    observe({ .Q$toConsole(input$jserr,'error','javascript error') })
...
###Maps###
{
    ...
    observe({
        if (is.null(input$jsSave)) { return(NULL) }
        qb$saveRO <- input$jsSave
    ...
    .Q$select <- function(query = NA) {
        print(query)
        if(is.na(query)) { cursor <- mongo.find(qbase,qb$path) }
        else { cursor <- mongo.find(qbase,qb$path) }
